<!--
@file src/lib/components/pension/state/StatementsCard.svelte
@kind component
...
<script lang="ts">
	...
	// Find the index of the latest statement by date
	...
	// Get previous statements sorted by date (newest first), excluding the latest
	...
	async function handleRemoveStatement(index: number) {
		...
		if (pensionId && statement?.id) {
			try {
				await pensionApi.deleteStatePensionStatement(pensionId, statement.id);
			} catch (error) {
...
<div class="space-y-6">
	...
	{#if statements.length > 0}
		...
		<div class="space-y-4">
			<h3 class="text-lg font-medium">
				{m.state_pension_latest_statement()} (<FormattedDate value={statements[li].statement_date} />)
			</h3>
	...
	{#if previousStatements.length > 0}
		<div class="space-y-4">
			<h3 class="text-lg font-medium">{m.state_pension_previous_statements()}</h3>
	...
	{#if statements.length === 0}
		...
		>
			<Plus class="h-4 w-4 mr-2" />
			{m.state_pension_add_first_statement()}
		</button>
	{/if}
</div>
...
>
	<div class="p-6">
		<h2 class="text-lg font-semibold">{m.state_pension_delete_statement_title()}</h2>
		<p class="mt-2 text-sm text-muted-foreground">{m.state_pension_delete_statement_confirm()}</p>
	...
	<div class="space-y-4">
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				<label class="block text-sm font-medium mb-1.5">
					{m.state_pension_statement_date()}
					<input
						type="date"
			...
			<div>
				...
				<CurrencyInput bind:value={statements[index].current_value} min={0} />
			</div>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div>
				...
				<CurrencyInput bind:value={statements[index].projected_monthly_amount} min={0} />
			</div>
		</div>

		<div>
			<label class="block text-sm font-medium mb-1.5">
				{m.state_pension_statement_note()}
				<textarea
					bind:value={statements[index].note}
					placeholder={m.state_pension_statement_note_placeholder()}
					rows="3"
