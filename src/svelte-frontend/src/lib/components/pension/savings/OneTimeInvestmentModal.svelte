<!--
@file src/lib/components/pension/savings/OneTimeInvestmentModal.svelte
@kind component
@purpose Steuert den Dialog für eine einmalige Einzahlung bei einer Sparplan-Pension.
-->
...
<script lang="ts">
	...
	function validate(): boolean {
		...
		if (!amount || amount <= 0) newErrors.amount = m.savings_one_time_amount_required();
		if (!investmentDate) newErrors.investment_date = m.savings_one_time_date_required();
		errors = newErrors;
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		submitting = true;
		try {
			await pensionApi.addSavingsPensionOneTimeInvestment(pensionId, {
				amount,
				investment_date: investmentDate,
				note: note.trim() || undefined
			});
			toastStore.success(m.savings_one_time_added());
			onSuccess?.();
			onClose();
		} catch (error) {
			console.error('Error adding one-time investment:', error);
			toastStore.error(error instanceof Error ? error.message : m.savings_one_time_failed());
		} finally {
			submitting = false;
		}
	}
...
>
	<form onsubmit={handleSubmit}>
		<div class="p-6 space-y-4">
			<div>
				<h2 class="text-lg font-semibold">{m.savings_add_one_time_investment()}</h2>
				<p class="text-sm text-muted-foreground mt-1">{pensionName}</p>
			</div>

			<!-- Amount -->
			<div class="space-y-1">
				<p class="text-sm font-medium">{m.savings_one_time_amount()}</p>
			...
			<!-- Date -->
			<div class="space-y-1">
				<label for="investment-date" class="text-sm font-medium">{m.savings_one_time_date()}</label>
				<input
					id="investment-date"
					type="date"
			...
			<!-- Note -->
			<div class="space-y-1">
				<label for="investment-note" class="text-sm font-medium">{m.savings_one_time_note()}</label>
				<textarea
					id="investment-note"
					bind:value={note}
					placeholder={m.savings_one_time_note_placeholder()}
					rows="2"
